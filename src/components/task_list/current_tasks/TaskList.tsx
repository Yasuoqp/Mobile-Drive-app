import React, {FC, useState} from 'react';
import s from '@/styles/task.module.scss';
import {Link, redirect} from 'react-router-dom';
import {ITask} from "@/model/ITask";


interface Props {
    tasks: ITask[];
}


const TaskList: FC<Props> = (props) => {
    const {tasks} = props;
    const [errorToken, setErrorToken] = useState<any>(tasks)


    function signInAgain() {
        localStorage.clear()
        location.reload()
    }

    function reloadPage() {
        location.reload()
    }

    return (
        <>
            {tasks.length >= 1 &&
                tasks.map((item: any) => {
                    return (
                        <div className={item.urgency == false ? s.defaultTask : s.do_not_rush_task}>
                            <Link to={`/tasks/${item.id}`} key={item.id} className={s.content}>
                                <div style={{width: '70%'}}>
                                    <h3 className={s.title}>{item.title}</h3>
                                    <span>{item.delivery_status}</span>
                                </div>
                                <div className={s.time_container}>
                                    <div>От : {item.date}</div>
                                    <span>До : {item.deadline}</span>
                                </div>
                            </Link>
                        </div>
                    );
                })}

            {errorToken.detail === "Invalid token." &&
              <div id="alert-additional-content-1"
                   style={{marginTop:'9em'}}
                       className="p-4 mb-4 text-blue-800 border border-blue-300 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400 dark:border-blue-800"
                   role="alert">
                  <div className="flex items-center">
                      <svg className="flex-shrink-0 w-4 h-4 me-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                           fill="currentColor" viewBox="0 0 20 20">
                          <path
                            d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z"/>
                      </svg>
                      <span className="sr-only">Info</span>
                      <h3 className="text-lg font-medium">Это информационное оповещение</h3>
                  </div>
                  <div className="mt-2 mb-4 text-sm">
                      Произошла ошибка загрузки данных. Попробуйте обновить страницу, если оповещение не пропадет
                      нажмите на повторную авторизацию
                  </div>
                  <div className="flex">
                      <button type="button"
                              onClick={reloadPage}
                              className="text-white bg-blue-800 hover:bg-blue-900 focus:ring-4 focus:outline-none focus:ring-blue-200 font-medium rounded-lg text-xs px-3 py-1.5 me-2 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                          <svg className="me-2 h-3 w-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                               fill="currentColor"
                               viewBox="0 0 20 14">
                              <path
                                d="M10 0C4.612 0 0 5.336 0 7c0 1.742 3.546 7 10 7 6.454 0 10-5.258 10-7 0-1.664-4.612-7-10-7Zm0 10a3 3 0 1 1 0-6 3 3 0 0 1 0 6Z"/>
                          </svg>
                          Обновить
                      </button>
                      <button type="button"
                              onClick={signInAgain}
                              className="text-blue-800 bg-transparent border border-blue-800 hover:bg-blue-900 hover:text-white focus:ring-4 focus:outline-none focus:ring-blue-200 font-medium rounded-lg text-xs px-3 py-1.5 text-center dark:hover:bg-blue-600 dark:border-blue-600 dark:text-blue-400 dark:hover:text-white dark:focus:ring-blue-800"
                              data-dismiss-target="#alert-additional-content-1" aria-label="Close">
                          Войти заново
                      </button>
                  </div>
              </div>
            }
        </>
    );
};

export default TaskList;